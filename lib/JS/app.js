...
//     countDownDate = new Date(countdowns[i].deadline).getTime();
// }


// Countdown nach folgendem Konzept speichern: const newDl = new Countdown('Feffe', 'Oct 19, 2021 15:06:00');

// Objekt entsprechend laden. Iteriert das Array Countdowns, befüllt den Maxwert, lädt kleinstes zuerst

//Über Button Links rechts navigieren können Fängt bei 0 an und zählt dann hoch, wenn man nach rechts klickt, runter nach links
//Dies ist an Array.Length gekoppelt. Wenn 0 dann Button nach links und rechts unsichtbar, Wenn Length erreicht, jeweiliger Button unsichtbar

// Angezeigtes Objekt löschen
    ...
    if(localStorage.getItem("storedDeadline") != "") {
        try {
            ...
            if(countdowns === null){
                ...
                document.getElementById("deadline").innerHTML = countdowns[currentCountdown].event;
                countDownDate = new Date(countdowns[currentCountdown].deadline).getTime();
            }
            countdownAmount = countdowns.length;
            console.log("Bin im Load bereich", countdowns, ' // Anzahl: ' + countdownAmount);
            showHideNavibuttons();
        } catch (error) {
...
function deleteCountdown() {
    alert("Diese Funktion wird noch programmiert");
}